...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-6 bg-gray-50 min-h-screen">
                ...
                <div class="flex flex-col md:flex-row md:items-end gap-4 mb-6">
                    ...
                    <div class="w-full md:w-1/4">
                        ...
                        <div class="relative" id="batch-dropdown">
                            ...
                            <!-- Dropdown Menu -->
                            <div v-if="isDropdownOpen" class="absolute z-10 mt-1 w-full rounded-md bg-white shadow-lg border border-gray-200 max-h-60 overflow-y-auto">
                                <div class="py-1">
                                    <button 
                                        @click="selectedBatch = ''; isDropdownOpen = false"
                    ...
                    <div class="w-full md:w-1/4">
                        ...
                        <div class="relative" id="remarks-dropdown">
                            ...
                            <!-- Dropdown Menu -->
                            <div v-if="isRemarksDropdownOpen" class="absolute z-10 mt-1 w-full rounded-md bg-white shadow-lg border border-gray-200 max-h-60 overflow-y-auto">
                                <div class="py-1">
                                    <button 
                                        @click="selectedRemarkSort = 'all'; isRemarksDropdownOpen = false"
                                        class="block w-full px-4 py-2 text-left text-sm hover:bg-gray-100"
                    ...
                    <div class="w-full md:w-auto">
                        <button 
                            @click="resetFilters" 
                            class="w-full md:w-auto px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50"
                        >
                            Reset Filters
                        </button>
                    </div>
                ...
                <div class="bg-white rounded-lg shadow border overflow-hidden">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-100">
                            <tr>
                                <th @click="toggleSort('date_distribute')" class="px-6 py-3 text-left text-xs font-semibold text-gray-600 cursor-pointer group">
                                    <div class="flex items-center">
                                        Date Distribute
                                        <ArrowUpDown class="ml-1 h-4 w-4 transition-all" :class="getSortIcon('date_distribute')" />
                                    </div>
                                </th>
                                <th @click="toggleSort('brand_name')" class="px-6 py-3 text-left text-xs font-semibold text-gray-600 cursor-pointer group">
                                    <div class="flex items-center">
                                        Brand Name
                                        <ArrowUpDown class="ml-1 h-4 w-4 transition-all" :class="getSortIcon('brand_name')" />
                                    </div>
                                </th>
                                <th @click="toggleSort('generic_name')" class="px-6 py-3 text-left text-xs font-semibold text-gray-600 cursor-pointer group">
                                    <div class="flex items-center">
                                        Generic Name
                                        <ArrowUpDown class="ml-1 h-4 w-4 transition-all" :class="getSortIcon('generic_name')" />
                                    </div>
                                </th>
                                <th @click="toggleSort('lot_number')" class="px-6 py-3 text-left text-xs font-semibold text-gray-600 cursor-pointer group">
                                    <div class="flex items-center">
                                        Lot/Batch
                                        <ArrowUpDown class="ml-1 h-4 w-4 transition-all" :class="getSortIcon('lot_number')" />
                                    </div>
                                </th>
                                <th @click="toggleSort('quantity')" class="px-6 py-3 text-left text-xs font-semibold text-gray-600 cursor-pointer group">
                                    <div class="flex items-center">
                                        Quantity
                                        <ArrowUpDown class="ml-1 h-4 w-4 transition-all" :class="getSortIcon('quantity')" />
                                    </div>
                                </th>
                                <th @click="toggleSort('expiration_date')" class="px-6 py-3 text-left text-xs font-semibold text-gray-600 cursor-pointer group">
                                    <div class="flex items-center">
                                        Expires
                                        <ArrowUpDown class="ml-1 h-4 w-4 transition-all" :class="getSortIcon('expiration_date')" />
                                    </div>
                                </th>
                                <th @click="toggleSort('remarks')" class="px-6 py-3 text-left text-xs font-semibold text-gray-600 cursor-pointer group">
                                    <div class="flex items-center">
                                        Remarks
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <tr v-for="item in filteredInventory" :key="item.id" class="hover:bg-gray-50 transition">
                            ...
                            <tr v-if="filteredInventory.length === 0">
                                <td colspan="8" class="px-6 py-6 text-center text-gray-500 text-sm">
                                    ...
                                    <button @click="resetFilters" class="text-blue-500 hover:underline ml-1">Reset filters</button>
                                </td>
                            </tr>
                        </tbody>
                    </table>
