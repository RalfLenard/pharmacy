...
const fetchDistributions = async () => {
    ...
    try {
        ...
        // If we have a distribution, set the selected lot number and distribution ID
        if (props.distribution) {
            const currentDist = distributions.value.find(d => d.id.toString() === form.distribution_id);
            if (currentDist) {
                selectedLotNumber.value = currentDist.inventory.lot_number;
...
watch(() => props.distribution, (newVal) => {
    if (newVal) {
        ...
        // Set the selected lot number and distribution ID if distributions are loaded
